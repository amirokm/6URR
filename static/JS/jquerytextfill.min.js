...
let meditationId;
let uuid;
const isCustom = pathSegments[1].toLowerCase() === 'meditations' || pathSegments[1].toLowerCase() === 'preview';
...
async function getMeditation() {
    ...
    if (pathSegments[1] === 'meditations') {
        ...
        meditation = await window.fetch(`/api/meditation/${meditationId}`, {
            headers: {
...
$(document).ready(function() {
    ...
    function resizeStar() {
        const lengthOfAnimation = messages === DEFAULT_MEDIATION ? 55000 : TIME_PER_MESSAGE * messages.length;
        $('.main-star').css('transition', `linear ${lengthOfAnimation / 1000}s transform`);
        $('.main-star').css({
    ...
    function initializeDisappear() {
        ...
        if (!isPreview && !startedMeditation) {
            ...
            window.fetch('https://freegeoip.app/json/').then((response)=>response.json()).then((geo)=>{
                $.ajax({
                    type: 'POST',
                    url: '/message',
                    data: {
        ...
        $('.main-star__thought-text').css({
            opacity: '0'
        });
        $('.main-star__thought-text').animate({
            opacity: '1'
        }, 500);
        $('.main-star__thought-wrapper').textfill();
        const offsetHeight = ($('.main-star__thought-wrapper').height() - $('.main-star__thought-text').height()) / 2;
        $('.main-star__thought-text').css({
            top: `${offsetHeight}px`
        });
        $('.input__thought').animate({
